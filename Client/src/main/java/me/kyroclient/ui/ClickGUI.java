...
public class ClickGUI extends GuiScreen
{
    ...
    public ClickGUI() {
        ...
        for (Module.Category c : Module.Category.values()) {
            panels.add(new Panel(c, (float)px, (float)py, (float)pwidth, (float)pheight));
            px += pwidth + 10;
        }
    }
    ...
    private void handle(int mouseX, int mouseY, final int key, final float partialTicks, final Handle handle) {
        final int toggled = KyroClient.clickGui.getColor().getRGB();
        final float scale = 2.0f / this.mc.gameSettings.guiScale;
        final int prevScale = this.mc.gameSettings.guiScale;
        GL11.glPushMatrix();
        if (this.mc.gameSettings.guiScale > 2 && !KyroClient.clickGui.scaleGui.isEnabled()) {
        ...
        if (handle == Handle.DRAW) {
            int blur = 0;
            final String selected = KyroClient.clickGui.blur.getSelected();
            switch (selected) {
                case "Low": {
                    blur = 4;
                    break;
        ...
        for (final Panel p : ClickGUI.panels) {
            ...
            final List<Module> list = (List<Module>) Module.getModulesByCategory(p.category).stream().sorted(Comparator.comparing(module -> Fonts.getPrimary().getStringWidth(module.getName()))).collect(Collectors.toList());
            Collections.reverse(list);
            for (final Module module2 : list) {
                if (module2.isDevOnly() && !KyroClient.isDev) {
                    continue;
                }
                Label_0942: {
                    switch (handle) {
                        case DRAW: {
                            ...
                            Fonts.getPrimary().drawSmoothCenteredString(module2.getName(), p.x + p.width / 2.0f, y + moduleHeight / 2.0f - Fonts.getPrimary().getHeight() / 2.0f, Color.white.getRGB());
                            break;
                        }
                        case CLICK: {
                        ...
                        if (setting instanceof ModeSetting) {
                            switch (handle) {
                                case DRAW: {
                                    ...
                                    Fonts.getPrimary().drawSmoothString(setting.name, p.x + 2.0f, y + moduleHeight / 2.0f - Fonts.getPrimary().getHeight() / 2.0f, Color.white.getRGB());
                                    Fonts.getPrimary().drawSmoothString(((ModeSetting)setting).getSelected(), p.x + p.width - Fonts.getPrimary().getStringWidth(((ModeSetting)setting).getSelected()) - 2.0, y + moduleHeight / 2.0f - Fonts.getPrimary().getHeight() / 2.0f, new Color(143, 143, 143, 255).getRGB());
                                    break;
                                }
                                case CLICK: {
                                    if (!this.isHovered(mouseX, mouseY, p.x + p.width - Fonts.getPrimary().getStringWidth(((ModeSetting)setting).getSelected()) - 2.0, y, moduleHeight, Fonts.getPrimary().getStringWidth(((ModeSetting)setting).getSelected()))) {
                                        break;
                                    }
                            ...
                            switch (handle) {
                                case DRAW: {
                                    ...
                                    Fonts.getPrimary().drawSmoothString(setting.name, p.x + 2.0f, y + moduleHeight / 2.0f - Fonts.getPrimary().getHeight() / 2.0f, Color.white.getRGB());
                                    Fonts.getPrimary().drawSmoothString(String.valueOf(((NumberSetting)setting).getValue()), p.x + p.width - 2.0f - Fonts.getPrimary().getStringWidth(String.valueOf(((NumberSetting)setting).getValue())), y + moduleHeight / 2.0f - Fonts.getPrimary().getHeight() / 2.0f, Color.white.getRGB());
                                    break;
                                }
                                case CLICK: {
                                    if (!this.isHovered(mouseX, mouseY, p.x, y, moduleHeight, p.width)) {
                            ...
                            final String value = (this.settingString == setting) ? (((StringSetting)setting).getValue() + "_") : ((((StringSetting)setting).getValue() == null || ((StringSetting)setting).getValue().equals("")) ? (setting.name + "...") : Fonts.getPrimary().trimStringToWidth(((StringSetting)setting).getValue(), (int)p.width));
                            Label_2455: {
                                switch (handle) {
                                    case DRAW: {
                                        RenderUtils.drawRect(p.x, y, p.x + p.width, y + moduleHeight, new Color(ClickGUI.background).brighter().getRGB());
                                        Fonts.getPrimary().drawCenteredString(value, p.x + p.width / 2.0f, y + moduleHeight / 2.0f - Fonts.getPrimary().getHeight() / 2.0f, (((StringSetting)setting).getValue() == null || (((StringSetting)setting).getValue().equals("") && this.settingString != setting)) ? new Color(143, 143, 143, 255).getRGB() : Color.white.getRGB());
                                        break;
                                    }
                                    case CLICK: {
                                        if (!this.isHovered(mouseX, mouseY, p.x, y, moduleHeight, p.width)) {
                            ...
                            switch (handle) {
                                ...
                                case DRAW: {
                                    RenderUtils.drawRect(p.x, y, p.x + p.width, y + moduleHeight, new Color(ClickGUI.background).brighter().getRGB());
                                    Fonts.getPrimary().drawCenteredString(setting.name, p.x + p.width / 2.0f, y + moduleHeight / 2.0f - Fonts.getPrimary().getHeight() / 2.0f, Color.white.getRGB());
                                    break;
                                }
                            }
                        }
                ...
                Label_3059: {
                    if ((handle == Handle.DRAW && y >= p.y) || (handle == Handle.CLICK && y >= p.y + p.height + 3.0f)) {
                        ...
                        switch (handle) {
                            case DRAW: {
                                RenderUtils.drawRect(p.x, y, p.x + p.width, y + moduleHeight, new Color(ClickGUI.background).brighter().getRGB());
                                Fonts.getPrimary().drawSmoothString("Key", p.x + 2.0f, y + moduleHeight / 2.0f - Fonts.getPrimary().getHeight() / 2.0f, Color.white.getRGB());
                                Fonts.getPrimary().drawSmoothString(keyText, p.x + p.width - Fonts.getPrimary().getStringWidth(keyText) - 2.0, y + moduleHeight / 2.0f - Fonts.getPrimary().getHeight() / 2.0f, new Color(143, 143, 143, 255).getRGB());
                                break;
                            }
                            case CLICK: {
                                if (!this.isHovered(mouseX, mouseY, p.x + p.width - Fonts.getPrimary().getStringWidth(keyText) - 2.0, y, moduleHeight, Fonts.getPrimary().getStringWidth(keyText))) {
                                    break;
                                }
                                switch (key) {
                                    case 0: {
            ...
            if (handle == Handle.DRAW) {
                ...
                Fonts.getSecondary().drawSmoothCenteredString(p.category.name, p.x + p.width / 2.0f, p.y + p.height / 2.0f - Fonts.getSecondary().getHeight() / 2.0f, Color.white.getRGB());
    ...
    private int getTotalSettingsCount(final Panel panel) {
        ...
        for (final Module module : Module.getModulesByCategory(panel.category)) {
            if (module.isDevOnly() && !KyroClient.isDev) {
                continue;
            }
            ++count;
            if (!module.extended) {
