...
public abstract class PlayerSPMixin extends AbstractClientPlayerMixin
{
    ...
    public void onUpdateWalkingPlayer() {
        ...
        if (this.isCurrentViewEntity()) {
            ...
            PlayerUtil.lastGround = event.onGround;
    ...
    public boolean isUsingItem(final EntityPlayerSP instance) {
        return !KyroClient.noSlow.isToggled() && (instance.isUsingItem());
    }
    ...
    public void onLivingUpdate(final CallbackInfo ci) {
        if (KyroClient.noSlow.isToggled()) {
            ...
            if (action == EnumAction.BLOCK) {
                ...
                movementInput.moveForward *= (float)KyroClient.noSlow.swordSlowdown.getValue();
                final MovementInput movementInput2 = this.movementInput;
                movementInput2.moveStrafe *= (float)KyroClient.noSlow.swordSlowdown.getValue();
            }
            else if (action == EnumAction.BOW) {
                final MovementInput movementInput3 = this.movementInput;
                movementInput3.moveForward *= (float)KyroClient.noSlow.bowSlowdown.getValue();
                final MovementInput movementInput4 = this.movementInput;
                movementInput4.moveStrafe *= (float)KyroClient.noSlow.bowSlowdown.getValue();
            }
            else if (action != EnumAction.NONE) {
                final MovementInput movementInput5 = this.movementInput;
                movementInput5.moveForward *= (float)KyroClient.noSlow.eatingSlowdown.getValue();
                final MovementInput movementInput6 = this.movementInput;
                movementInput6.moveStrafe *= (float)KyroClient.noSlow.eatingSlowdown.getValue();
            }
        }
        if (KyroClient.freeCam.isToggled()) {
            this.noClip = true;
        }
    }
