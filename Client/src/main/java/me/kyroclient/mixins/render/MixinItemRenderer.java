...
public abstract class MixinItemRenderer {
        ...
        if (this.itemToRender != null) {
            ...
            if (this.itemToRender.getItem() instanceof ItemMap) {
                ...
                switch (enumaction) {
                    ...
                    case BLOCK: {
                        if (KyroClient.animations.isToggled()) {
                            final String selected = KyroClient.animations.mode.getSelected();
                            switch (selected) {
                                case "1.7": {
                                ...
                                case "spin":
                                case "vertical spin": {
                                    this.transformFirstPersonItem(f, KyroClient.animations.showSwing.isEnabled() ? f2 : 0.0f);
                                    this.doBlockTransformations();
                                    break;
                                }
                                case "long hit": {
    ...
    private void transformFirstPersonItem(final float equipProgress, final float swingProgress) {
        final float size = (float) KyroClient.animations.size.getValue();
        final float x = (float)KyroClient.animations.x.getValue();
        final float y = (float)KyroClient.animations.y.getValue();
        final float z = (float)KyroClient.animations.z.getValue();
