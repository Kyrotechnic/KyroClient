...
public class Interfaces extends Module
{
    ...
    private void renderScoreboard(final ScoreObjective p_180475_1_, final ScaledResolution p_180475_2_, final boolean customFont) {
        ...
        final int fontHeight = customFont ? (Fonts.getPrimary().getHeight() + 2) : KyroClient.mc.fontRendererObj.FONT_HEIGHT;
        ...
        final float arrayHeight = KyroClient.clickGui.getHeight();
        float j1 = p_180475_2_.getScaledHeight() / 2.0f + i1 / 3.0f;
        if (KyroClient.clickGui.arrayList.isEnabled()) {
    ...
    private void drawString(String s, final float x, final float y, final int color, final boolean customFont) {
        if (KyroClient.nickHider.isToggled() && s.contains(KyroClient.mc.getSession().getUsername())) {
            s = s.replaceAll(KyroClient.mc.getSession().getUsername(), KyroClient.nickHider.nick.getValue());
        }
        if (customFont) {
            Fonts.getPrimary().drawSmoothStringWithShadow(s, x, y, Color.white.getRGB());
        }
        else {
            KyroClient.mc.fontRendererObj.drawString(s, (int)x, (int)y, color);
        }
    }

    private float getStringWidth(final String s, final boolean customFont) {
        return customFont ? ((float)Fonts.getPrimary().getStringWidth(s)) : ((float)KyroClient.mc.fontRendererObj.getStringWidth(s));
    }

    @Override
    public void assign()
    {
        KyroClient.interfaces = this;
    }
}
